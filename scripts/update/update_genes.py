        ...
        for stable_id in unique_stable_id_2_gene_symbol:
            ...
            try:
                ...
                if g2p_gene_symbol != new_gene_symbol:
                    ...
                    if (
                        looks_like_identifier(new_gene_symbol)
                        and not looks_like_identifier(g2p_gene_symbol)
                    ) or new_gene_symbol in g2p_gene_ids:
                        ...
                        # Write to report
                        wr.write(
                            f"ADD SYNONYM: locus_id = {g2p_gene_ids[stable_id]['locus_id']} add locus_attrib {new_gene_symbol}\n\n"
                        )
                    else:
                        # Update gene symbol
                        cursor.execute(
                            sql_update_gene,
                            [new_gene_symbol, g2p_gene_ids[stable_id]["locus_id"]],
                        )
                        ...
                        wr.write(
                            f"ADD SYNONYM: locus_id = {g2p_gene_ids[stable_id]['locus_id']} add locus_attrib {g2p_gene_symbol}\n"
                        )

                    # Compare the coordinates
                    if (
                        g2p_gene_ids[stable_id]["locus_start"]
                        != unique_stable_id_2_gene_symbol[stable_id]["start"]
                        or g2p_gene_ids[stable_id]["locus_end"]
                        != unique_stable_id_2_gene_symbol[stable_id]["end"]
                        or g2p_gene_ids[stable_id]["locus_chr"]
                        != sequence_chrs[
                            unique_stable_id_2_gene_symbol[stable_id]["chr"]
                        ]
                    ):
                        ...
                        db.commit()
                        wr.write(
                            f"UPDATE COORD: locus_id = {g2p_gene_ids[stable_id]['locus_id']} (previous {g2p_gene_ids[stable_id]['locus_chr']}:{g2p_gene_ids[stable_id]['locus_start']}-{g2p_gene_ids[stable_id]['locus_end']})\n"
                        )
        ...
        for symbol in mappings:
            ...
            try:
                ...
                if len(mappings[symbol]["hgnc_id"]) == 1:
                    if (
                        "hgnc_id" in g2p_data
                        and mappings[symbol]["hgnc_id"][0] != g2p_data["hgnc_id"]
                    ):
                        update_g2p_identifier(
                            "HGNC_update",
                            mappings[symbol]["hgnc_id"][0],
                            g2p_data["locus_id"],
                            db_host,
                            db_port,
                            db_name,
                        ...
                        wr.write(
                            f"UPDATE HGNC: locus_id = {g2p_data['locus_id']} gene symbol {symbol} {mappings[symbol]['hgnc_id'][0]}\n"
                        )
                        ...
                        wr.write(
                            f"ADD HGNC: locus_id = {g2p_data['locus_id']} gene symbol {symbol} {mappings[symbol]['hgnc_id'][0]}\n"
                        )
                ...
                if "omim_id" in mappings[symbol]:
                    ...
                    if (
                        len(omim_ids) == 1
                        and omim_ids[0] != ""
                        and "omim_id" in g2p_data
                        and omim_ids[0] != g2p_data["omim_id"]
                    ):
                        update_g2p_identifier(
                            "OMIM_update",
                            omim_ids[0],
                        ...
                        wr.write(
                            f"UPDATE OMIM ID: locus_id = {g2p_data['locus_id']} gene symbol {symbol} {omim_ids[0]}\n"
                        )
                    elif (
                        "omim_id" not in g2p_data
                        and omim_ids[0] != ""
                        and len(omim_ids) == 1
                    ):
                        update_g2p_identifier(
                        ...
                        wr.write(
                            f"ADD OMIM ID: locus_id = {g2p_data['locus_id']} gene symbol {symbol} {omim_ids[0]}\n"
                        )
                ...
                if "prev_symbol" in mappings[symbol]:
                    ...
                    for gene_syn in prev_symbols:
                        if (
                            gene_syn.upper() not in g2p_data["synonyms"]
                            and gene_syn not in g2p_data["synonyms"]
                        ) and gene_syn != "":
                            add_gene_synonym(
                                gene_syn,
                                g2p_data["locus_id"],
                            ...
                            wr.write(
                                f"ADD GENE PREV SYMBOL: locus_id = {g2p_data['locus_id']} gene symbol {symbol} prev symbol {gene_syn}\n"
                            )
...
def update_meta(
    db_host: str, db_port: int, db_name: str, user: str, password: str, version: str
) -> None:
    """
    ...
    cursor.execute(
        sql,
        ["locus_gene_update", datetime.now(), 0, description, version, source_id[0]],
    )
    db.commit()
    db.close()
