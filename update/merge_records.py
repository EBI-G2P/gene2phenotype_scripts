    ...
    with open(file_not_merged, "w") as wr, open(file_to_merge, "w") as wr_merge:
        for line in records:
            ...
            if not g2p_id_to_keep.startswith("G2P"):
                wr.write(
                    f"Invalid G2P ID\t{g2p_id_to_keep}\t{line['g2p ids to merge']}\n"
                )
                continue
            ...
            try:
                ...
                if lgd_response.status_code == 200:
                    ...
                    if gene != response_json["locus"]["gene_symbol"]:
                        wr.write(
                            f"Gene doesn't match\t{g2p_id_to_keep}\t{line['g2p ids to merge']}\n"
                        )
                    elif disease_name != response_json["disease"]["name"]:
                        wr.write(
                            f"Disease doesn't match\t{g2p_id_to_keep}\t{line['g2p ids to merge']}\n"
                        )
                    elif genotype != response_json["genotype"]:
                        wr.write(
                            f"Genotype doesn't match\t{g2p_id_to_keep}\t{line['g2p ids to merge']}\n"
                        )
                    elif mechanism != response_json["molecular_mechanism"]["mechanism"]:
                        wr.write(
                            f"Mechanism doesn't match\t{g2p_id_to_keep}\t{line['g2p ids to merge']}\n"
                        )
                    else:
                        # Create output
                    ...
                    print(
                        f"Failed fetching record {g2p_id_to_keep}. Status code: {lgd_response.status_code}"
                    )
...
def merge_records(
    records_to_merge: list, api_url: str, cookies: requests.cookies.RequestsCookieJar
) -> None:
    """
    ...
    try:
        response_update = requests.post(
            merge_url, json=records_to_merge, cookies=cookies
        )
    except Exception as e:
        print("Error:", e)
    else:
...
def main():
    ...
    parser.add_argument(
        "--config", required=True, help="Config file with the G2P API URL"
    )
    parser.add_argument(
        "--input-file",
        required=True,
    ...
    parser.add_argument(
        "--dryrun",
        action="store_true",
        help="Option to test the which records are going to be merged",
    )
