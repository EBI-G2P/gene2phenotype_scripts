...
"""
    Imports/updates gene-disease associations from OMIM (Ensembl) and Mondo
...
def get_g2p_meta(db_host, db_port, db_name, user, password):
    ...
    for row in data:
        ...
        if (
            source_name not in meta_info
            or date_update > meta_info[source_name]["date_update"]
        ):
...
def get_mim_gene_diseases(db_host, db_port, db_name, user, password):
    ...
    for row in data:
        ...
        if row[1] not in gene_diseases.keys():
            gene_diseases[row[1]] = [{"mim_id": row[2], "disease": disease}]
        else:
            gene_diseases[row[1]].append({"mim_id": row[2], "disease": disease})

    db.close()
    return gene_diseases


def insert_mim_gene_diseases(
    db_host, db_port, db_name, user, password, gene_diseases, version
):
    ...
    for stable_id, gd_info in gene_diseases.items():
        ...
        if gene_id is not None:
            for info in gd_info:
                cursor.execute(
                    sql_insert,
                    [gene_id, info["disease"], info["mim_id"], source_ids["OMIM"]],
                )
    ...
    for tmp_file in listdir("tmp/"):
        with open("tmp/" + tmp_file, "r") as fh:
            ...
            for event, elem in ET.iterparse(fh, events=("start", "end")):
                ...
                if (
                    event == "start"
                    and elem.tag == "{http://www.w3.org/2002/07/owl#}Class"
                ):
                    ...
                    if start_class == 0:
                        ...
                        for i in elem.iter():
                            ...
                            if (
                                ...
                                and i.text is not None
                            ):
                                is_element = i.text

                            # Get disease name
                            if (
                                i.tag == "{http://www.w3.org/2000/01/rdf-schema#}label"
                                and i.text is not None
                            ):
                                disease_name = i.text
                            ...
                            if (
                                ...
                                and i.attrib is not None
                                and "{http://www.w3.org/1999/02/22-rdf-syntax-ns#}resource"
                                in i.attrib.keys()
                                and "hgnc"
                                in i.attrib[
                            ...
                            if (
                                is_element is not None
                                and disease_name is not None
                                and not disease_name.startswith("obsolete")
                            ):
                            ...
                            # This is a special case
                                ...
                                is_element is not None
                                and disease_name is None
                                and disease_synonym is not None
                            ):
                                mondo_ids.append(is_element)
                ...
                if (
                    is_element is not None
                    and disease_name is not None
                    and is_element not in mondo_diseases
                ):
                ...
                if (
                    event == "end"
                    and elem.tag == "{http://www.w3.org/2002/07/owl#}Restriction"
                    and is_element is None
                    and len(mondo_ids) >= 1
                ):
                    ...
                    for i in elem.iter():
                        if (
                            i.tag == "{http://www.w3.org/2002/07/owl#}someValuesFrom"
                            and "{http://www.w3.org/1999/02/22-rdf-syntax-ns#}resource"
                            in i.attrib.keys()
                            and "hgnc"
                            in i.attrib[
                ...
                if (
                    is_element is not None
                    and disease_name is not None
                    and hgnc_id is not None
                    and is_element not in results
                ):
...
def insert_mondo_gene_diseases(
    db_host, db_port, db_name, user, password, gene_diseases, mondo_version
):
    """
    ...
    for gd, gd_info in gene_diseases.items():
        ...
        if gene_id is not None:
            for info in gd_info:
                cursor.execute(
                    sql_insert, [gene_id, info["disease"], info["mondo_id"], source_id]
                )
...
def populate_mondo_all_diseases(
    db_host, db_port, db_name, user, password, mondo_all_diseases
):
...
def fetch_mondo_version(file):
    ...
    with open(file, "r") as fh:
        for event, elem in ET.iterparse(fh, events=("start", "end")):
            if (
                event == "start"
                and elem.tag == "{http://www.w3.org/1999/02/22-rdf-syntax-ns#}RDF"
            ):
                for i in elem.iter():
...
def main():
    ...
    parser.add_argument(
        "--config",
        required=True,
        help="Config file with details to Ensembl and G2P databases",
    )
    parser.add_argument(
        "--skip_omim",
        dest="omim",
        action="store_false",
        help="Skip importing OMIM gene-disease data (default: off)",
    )
    parser.add_argument(
        "--skip_mondo",
        dest="mondo",
        action="store_false",
        help="Skip importing Mondo gene-disease data (default: off)",
    )
    parser.add_argument(
        "--mondo_file", default="", help="MONDO file in the format: owl"
    )
    ...
    g2p_user = config["g2p_database"]["user"]
    g2p_password = config["g2p_database"]["password"]

    # Fetch gene-disease import info from G2P
    g2p_meta_info = get_g2p_meta(
    ...
    if args.omim:
        ...
        if "Ensembl" in g2p_meta_info:
            print(
                f"INFO: current OMIM data is from Ensembl {g2p_meta_info['Ensembl']['data_version']}"
            )
    ...
    if args.mondo:
        ...
        if mondo_version is None:
            sys.exit(
                f"ERROR: could not detect Mondo version from input file '{mondo_file}'"
            )
        ...
        if "Mondo" in g2p_meta_info:
            g2p_mondo_version_obj = datetime.datetime.strptime(
                g2p_meta_info["Mondo"]["data_version"], "%Y-%m-%d"
            )
        ...
        for mondo, data in mondo_gene_diseases.items():
            if data["hgnc_id"] not in new_mondo_data:
                new_mondo_data[data["hgnc_id"]] = [
                    {"mondo_id": mondo, "disease": data["disease"]}
                ]
            else:
                new_mondo_data[data["hgnc_id"]].append(
                    {"mondo_id": mondo, "disease": data["disease"]}
                )

        # Insert Mondo gene-disease data into G2P db
        print("Inserting Mondo gene-disease associations into G2P...")
        insert_mondo_gene_diseases(
            g2p_db_host,
            g2p_db_port,
            g2p_db_name,
            g2p_user,
